...
let exportDbToJson = async(() => {
    ...
    const Geological = mongoose.model('Geological')
    let objectModelArr = []
    try {
        objectModelArr = await (Geological.find({"maDeTai.maDeTai": maDeTaiToExport, deleted_at: null}).exec())
        // objectModelArr = await (Geological.find({_id: mongoose.Types.ObjectId('5e5f304e803e1703bdaba5a3')}).exec())
        // objectModelArr = await (Geological.findById('5e5f304e803e1703bdaba5a3').exec())
    } catch (e) {
        console.log(e);
    }
    ...
    fs.writeFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-dia-chat-${convertFileName(maDeTaiToExport)}.json`), JSON.stringify(result, null, 2))
...
function generateCsvFile() {
    let data = JSON.parse(fs.readFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-dia-chat-${convertFileName(maDeTaiToExport)}.json`)))
    ...
    fs.writeFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-dia-chat-${convertFileName(maDeTaiToExport)}.csv`), csvContent)
    console.log('done');
    mongoose.connection.close();
}
