...
let exportDbToJson = async(() => {
    ...
    const Paleontological = mongoose.model('Paleontological')
    let objectModelArr = []
    try {
        objectModelArr = await (Paleontological.find({"maDeTai.maDeTai": maDeTaiToExport, deleted_at: null}).exec())
        // objectModelArr = await (Paleontological.find({_id: mongoose.Types.ObjectId('5e5f304e803e1703bdaba5a3')}).exec())
        // objectModelArr = await (Paleontological.findById('5e5f304e803e1703bdaba5a3').exec())
    } catch (e) {
        console.log(e);
    }
    ...
    fs.writeFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-co-sinh-${convertFileName(maDeTaiToExport)}.json`), JSON.stringify(result, null, 2))
...
function generateCsvFile() {
    let data = JSON.parse(fs.readFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-co-sinh-${convertFileName(maDeTaiToExport)}.json`)))
    ...
    fs.writeFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-co-sinh-${convertFileName(maDeTaiToExport)}.csv`), csvContent)
    console.log('done');
    mongoose.connection.close();
}
