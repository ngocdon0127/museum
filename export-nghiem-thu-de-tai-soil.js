...
let exportDbToJson = async(() => {
    ...
    fs.writeFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-tho-nhuong-${convertFileName(maDeTaiToExport)}.json`), JSON.stringify(result, null, 2))
...
function generateCsvFile() {
    let data = JSON.parse(fs.readFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-tho-nhuong-${convertFileName(maDeTaiToExport)}.json`)))
    ...
    fs.writeFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-tho-nhuong-${convertFileName(maDeTaiToExport)}.csv`), csvContent)
    console.log('done');
    mongoose.connection.close();
}
