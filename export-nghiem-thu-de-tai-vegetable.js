...
global.myCustomVars.models['thuc-vat'].PROP_FIELDS = PROP_FIELDS;
global.myCustomVars.models['thuc-vat'].PROP_FIELDS_OBJ = PROP_FIELDS_OBJ;
global.myCustomVars.models['thuc-vat'].UPLOAD_DESTINATION = UPLOAD_DESTINATION;
...
global.myCustomVars.models['thuc-vat'].bundle = bundle;
...
let exportDbToJson = async(() => {
    ...
    let objectModelArr = []
    try {
        objectModelArr = await (Vegetable.find({"maDeTai.maDeTai": maDeTaiToExport, deleted_at: null}).exec())
        // objectModelArr = await (Vegetable.find({_id: mongoose.Types.ObjectId('5e5f304e803e1703bdaba5a3')}).exec())
    ...
    fs.writeFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-vegetable-${convertFileName(maDeTaiToExport)}.json`), JSON.stringify(result, null, 2))
...
function generateCsvFile() {
    let data = JSON.parse(fs.readFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-vegetable-${convertFileName(maDeTaiToExport)}.json`)))
    ...
    fs.writeFileSync(path.join(__dirname, 'exports', `result-export-nghiem-thu-de-tai-vegetable-${convertFileName(maDeTaiToExport)}.csv`), csvContent)
    console.log('done');
}
